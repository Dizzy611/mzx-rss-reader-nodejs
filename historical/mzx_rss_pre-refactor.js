...
function format_review(title, link) {
	var spacesplit = title.split(" wrote a review for ");
	var author = spacesplit.shift();
	var rest = spacesplit.join(" wrote a review for ");
	return("***" + author + "*** wrote a review for " + rest + " " + link);
}
...
client.on('ready', async () => { 
	...
	var mzx = client.channels.cache.find(channel => channel.name === "mzx");
	console.log(`Found` + mzx);
	console.log(`Finding mzx-development...`);
	var mzxdev = client.channels.cache.find(channel => channel.name === "mzx-development");
	...
	var interval = setInterval(queue_send, 30*1000, mzx, mzxdev);
	console.log(`Initialization Complete`);
});

var interval = setInterval(feed_check, 60*1000);
